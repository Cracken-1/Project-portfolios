...
// Get patient details
$patient = $database->query("SELECT * FROM patient WHERE pid = '$pid'")->fetch_assoc();
if (!$patient) {
    header("location: billing.php");
    exit();
}
...
// Get patient's billing details
$billing_details = $database->query("
    SELECT i.*, d.docname, d.specialization, a.appodate, a.appotime
    FROM invoices i
...
<head>
    ...
    <style>
        ...
        .btn-view, .btn-print, .btn-statement {
            padding: 6px 12px;
            border-radius: 4px;
            font-size: 0.85rem;
        ...
        .btn-view i, .btn-print i, .btn-statement i {
            margin-right: 5px;
        }
    </style>
</head>
        ...
        <div class="billing-container">
            ...
            <div class="patient-header">
                <div class="patient-info">
                    <h2><i class="fas fa-user-injured"></i> <?php echo htmlspecialchars($patient['pname']); ?></h2>
                    <p>
                        <?php 
                        echo htmlspecialchars($patient['pname']) . ' | ' . 
                             htmlspecialchars($patient['pphoneno']) . ' | ' . 
                             htmlspecialchars($patient['pemail']);
                        ?>
                    </p>
                </div>
                <div>
                    <a href="generate_patient_statement.php?pid=<?php echo $pid; ?>" class="btn-statement" target="_blank">
                ...
                <tbody>
                    <?php foreach ($billing_details as $invoice): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($invoice['specialization']); ?></td>
